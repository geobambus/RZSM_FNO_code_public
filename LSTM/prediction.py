"""Apply a bundled LSTM ensemble to point-scale SSM records."""
...
def _scaled_static_values(row, scaler):
    ...
    if feature_names != list(STATIC_FEATURES):
        raise ValueError("Bundle scaler does not use the ordered LSTM features.")
...
def _write_prediction_file(
    ...
    try:
        ...
        with nc.Dataset(temporary_file, "w", format="NETCDF4") as output:
            ...
            ssm_variable = output.createVariable(
                f"{product}_SSM", "f4", ("time",), fill_value=np.nan
            )
            ssm_variable[:] = ssm
            ssm_variable.units = "m3 m-3"
            ssm_variable.long_name = f"{product} surface soil moisture"
            ...
            prediction.long_name = "Mean LSTM RZSM prediction across model seeds"
            ...
            uncertainty.long_name = (
                "Sample standard deviation of LSTM predictions across model seeds"
            )
            uncertainty.ddof = 1
