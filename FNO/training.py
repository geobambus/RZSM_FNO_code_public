"""Optuna optimization and deterministic ensemble training for FNO."""
...
from .settings import MODEL_SEEDS, OPTUNA_SEED, validate_product
...
class PixelLoss(nn.Module):
    ...
    def forward(self, prediction, target, pixel_ids):
        ...
        pixel_ids = torch.as_tensor(pixel_ids, device=prediction.device).reshape(-1)
        ...
        reconstruction = sum_by_group(
            (self.smooth_l1(prediction_safe, target_safe) * valid_float).sum(dim=1)
        ) / safe_counts
        ...
        prediction_squares = sum_by_group(prediction_centered.square().sum(dim=1))
        ...
        denominator = torch.sqrt(prediction_squares.clamp_min(epsilon)) * torch.sqrt(
            target_squares.clamp_min(epsilon)
        )
        ...
        return (group_loss * group_weights).sum() / group_weights.sum().clamp_min(1.0)


class EarlyStopping:
    """Retain the best weights and stop after a non-improvement count."""
    ...
    with torch.no_grad():
        for dataset in datasets:
            ...
            for dynamic, static, target, pixel_ids in loader:
                ...
                loss = loss_function(prediction, target.to(device), pixel_ids)
                losses.append((float(loss.item()), int(torch.unique(pixel_ids).numel())))
...
def fit_model(
    ...
    """Fit one FNO using the fixed temporal/spatial validation experiment."""
    ...
    loss_function = PixelLoss().to(device)
    ...
    for epoch in range(int(num_epochs)):
        ...
        for dynamic, static, target, pixel_ids in training_loader:
            ...
            noisy_static = static * keep + torch.randn_like(static) * 0.05 * keep
            prediction = model(dynamic, noisy_static)
            loss = loss_function(prediction, target, pixel_ids)
            if not torch.isfinite(loss):
                raise FloatingPointError(
                    f"FNO {product} produced non-finite training loss at epoch {epoch}."
                )
            loss.backward()
    ...
    if early_stopping.best_weights is None:
        raise RuntimeError(f"Training did not produce valid FNO weights for {product}.")
...
def optimize_hyperparameters(
    ...
    *,
    ...
    product = validate_product(product)
    scaler = compute_scaler(pd.read_csv(input_csv))
    ...
    def objective(trial):
        hyperparameters = {
            ...
            "modes": trial.suggest_int("modes", 4, 12),
...
def train_ensemble(
    ...
    *,
    ...
    """Train every deterministic seed and return state dictionaries in order."""
    product = validate_product(product)
...
def save_ensemble_bundle(output_file, ensemble, scaler, hyperparameters):
    """Atomically save only the fields required for FNO prediction."""
    payload = {
...
def load_ensemble_bundle(bundle_file):
    """Load and validate one prediction-ready FNO ensemble bundle."""
    bundle = torch.load(bundle_file, map_location="cpu", weights_only=True)
    required = {"hyperparameters", "scaler", "state_dicts"}
    missing = required.difference(bundle)
    if missing:
        raise RuntimeError(f"FNO bundle is missing fields: {sorted(missing)}")
    if not bundle["state_dicts"]:
        raise RuntimeError("FNO bundle contains no trained models.")
    return bundle
